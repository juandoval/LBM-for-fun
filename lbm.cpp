...
(9) discrete velocities e_i in D2Q9:
e_0 = (0, 0)
e_1 = (1, 0)
e_2 = (0, 1)
e_3 = (-1, 0)
...
(9) weights w_i of D2Q9 (They must sum to 1)
...
Equilibrium distribution f_i^eq(ρ, u) in D2Q9 is given by the formula:
...
What is the BGK collision step? What does ω (or τ) control physically?
...
How do you recover ρ and u from f_i?
...
void equilibrium(double f[9], double density, double velocity_x, double velocity_y) {
    for (int i = 0; i < 9; ++i) {
        ...
        // how aligned is direction i with the flow?
        // e_i (discrete direction vector) · u (macroscopic flow velocity at that node)
        double eu = e_x[i] * velocity_x + e_y[i] * velocity_y; 

        // total speed squared
        double uu = SQ(velocity_x) + SQ(velocity_y); // u · u
        
        // for bucket i, compute the equilibrium distribution where:
        // weight[i] * density: base amount for this direction at equilibrium
        // + eu/cs2: bias due to alignment with the flow
        // + SQ(eu)/(2.0*SQ(cs2)): second order correction (strengthens the bias)
        // - uu/(2.0*cs2): speed correction (ensures mass conservation)
...
void collide(double f[NX][NY][9], double tau, bool solid[NX][NY],
             double body_force_x, double body_force_y) {
    ...
    for (int x = 0; x < NX; ++x) {
        for (int y = 0; y < NY; ++y) {
            ...
            // Shift by F*tau/rho so the BGK (1/tau) factor cancels → effective force = F.
            // Naive shift F/rho injects F/tau momentum per step (wrong by factor tau).
            velocity_x += body_force_x * tau / density;
            velocity_y += body_force_y * tau / density;
            double f_eq[9];
